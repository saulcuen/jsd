...
void telemetry_header() {
  ...
  fprintf(file, "status, ticks, ");
  fprintf(file, "voltage_hv, voltage_lv, voltage_12v, ");
  fprintf(file, "temp_ambient, temp_actuator, ");
  fprintf(file, "humidity, pressure, brake_current, brake_cc_val,");
  fprintf(file, "cmd, ");
  fprintf(file, "\n");
}
...
void print_info(void* self) {
  ...
  MSG("voltage_hv:    %u", state->voltage_hv);
  MSG("voltage_lv:    %u", state->voltage_lv);
  MSG("voltage_12v:   %u", state->voltage_12v);
  MSG("temp_ambient:  %u", state->temp_ambient);
  MSG("temp_actuator: %u", state->temp_actuator);
  MSG("humidity:      %u", state->humidity);
  MSG("pressure:      %u", state->pressure);
  MSG("brake_current: %u", state->brake_current);
  MSG("brake_cc_val:  %u", state->brake_cc_val);
  MSG("cmd:           %u", state->cmd);
  MSG("");
}
