...
void jsd_jed0200_read(jsd_t* self, uint16_t slave_id) {
  ...
  // Indicates if txpdo packed struct matches the fixed PDO mapping on device TODO
  assert(self->ecx_context.slavelist[slave_id].Obytes == sizeof(jsd_jed0200_txpdo_t));

  // Indicates if rxpdo packed struct matches the fixed PDO mapping on device TODO
  assert(self->ecx_context.slavelist[slave_id].Ibytes == sizeof(jsd_jed0200_rxpdo_t));
